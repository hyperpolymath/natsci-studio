...
# setup.sh — Universal setup script for natsci-studio
#
...
# ── Install just ──
# Install just with the detected package manager unless it is already available.
# Return failure if the package manager is unsupported or just remains unavailable.
install_just() {
...
# ── Main ──
main() {
    printf "%s=== natsci-studio Setup ===%s\n\n" "$BOLD" "$RESET"
    ...
    if [ ! -f "Justfile" ] && [ ! -f "justfile" ]; then
        ...
        info "Clone first: git clone https://github.com/hyperpolymath/natsci-studio.git"
        info "Then: cd natsci-studio && ./setup.sh"
        exit 1
    fi
